 ...
 */
import { formatNumber, generateReferralCode, getISTDate } from '@/app/(tabs)/Home/home.helpers';
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTotalMiners(prevMiners => {
        // Randomly add 0-2 users to simulate growth
        const newUsers = Math.floor(Math.random() * 3);
        const currentMiners = prevMiners + newUsers;

        // Update LSN Mined based on real-time active miners
        // Logic: More miners = Faster network mining rate
        // Simulating ~0.05 LSN mined per user per interval
        setLsnMined(prevLsn => {
  ...
  const validReferralsCount = referrals.filter(r => r.status === 'active' && r.consecutiveDays >= 3).length;
  ...
  // Final Rate Calculation (Realtime)
  // If Boost is active, we add ONLY the base rate (3.15) to the current active mining rate.
  // Formula: (Base Rate + Referral Boost) + (Boost Active ? Base Rate : 0)
  ...
  const handleShareInvite = async () => {
    try {
      await Share.share({
        message: `Join me on Lassan App! Use my referral code ${referralCode} to get a bonus. Download now!`,
      });
    } catch {
  ...
  useEffect(() => {
    ...
    if (lastClaimedDate === today) {
      setIsDailyBonusAvailable(false);
    } else {
  ...
  const updateMiningStreak = () => {
    ...
    if (lastMiningDate) {
      ...
      if (diffDays === 1) {
        ...
        setMiningStreak(1); // Streak broken
      }
    } else {
      setMiningStreak(1); // First time
    }
  ...
  const triggerAd = (action: string) => {
    ...
    setTimeout(() => {
      ...
      handleAdReward(action);
    }, 3000);
  };

  const handleAdReward = (action: string) => {
    if (action === 'mining') {
      setPowerOn(true);
      updateMiningStreak();
    } else if (action === 'daily_bonus') {
      const reward = Math.floor(Math.random() * 6) + 10; // Random between 10-15
      setBalance(prev => prev + reward);
      showInfoAlert(t('success'), `${t('bonus_claimed_msg')}: +${reward} LSN`);
      setLastClaimedDate(getISTDate());
      setActiveModal(null);
    } else if (action === 'weekly_bonus') {
      const reward = Math.floor(Math.random() * 31) + 100; // Random between 100-130
      setBalance(prev => prev + reward);
      showInfoAlert("Success", `Weekly Bonus Claimed: +${reward} LSN`);
      setMiningStreak(0); // Reset streak after claim
      setActiveModal(null);
    ...
    if (status === 'pending') {
      try {
        ...
        if (supported) {
          ...
          setSocialTasks(prev => prev.map(t => t.id === id ? { ...t, status: 'verifying' } : t));
        } else {
      ...
      setSocialTasks(prev => prev.map(t => t.id === id ? { ...t, status: 'completed' } : t));
      setBalance(prev => prev + reward);
  ...
  const handlePowerPress = () => {
    if (powerOn) {
      // Optional: Ask confirmation to stop mining? For now just toggle off
      setPowerOn(false);
    } else {
      // Start Mining -> Watch Ad First
      triggerAd('mining');
    }
  };
  ...
  const handleBoostPress = () => {
    ...
    // If boost is active, allow opening to see the timer
    if (boostActive) {
    ...
    if (boostsUsedToday >= 5) {
      showInfoAlert("Limit Reached", "Daily Boost Limit Reached (5/5)");
      return;
    }

    if (lastBoostTime && (now - lastBoostTime < cooldownTime)) {
      const remaining = Math.ceil((cooldownTime - (now - lastBoostTime)) / (60 * 1000)); // minutes
    ...
    if (powerOn) {
      timer = setInterval(() => {
        ...
        setTimeLeft(prev => {
          ...
          if (h === 0 && m === 0 && s === 0) {
             setPowerOn(false); // Stop mining when time is up
             return "23:59:59";
          }
          let newS = s - 1;
          let newM = m;
        ...
        setBalance(prevBalance => {
            // Use the calculated finalMiningRate which includes referrals + boost logic
            // finalMiningRate is calculated in the component body based on state
      ...
      Animated.loop(
        Animated.sequence([
          Animated.timing(breatheAnim, {
            toValue: 1.08, // Slightly reduced scale for elegance
            duration: 2000,
            useNativeDriver: true,
  ...
  // We calculate exact Y positions based on the Flexbox layout logic to ensure SVG lines connect perfectly.
  
  // 1. Header Bottom Y
  // Header = marginTop(2) + padding(5) + balanceDisplay(height ~48 + padding 24) ~ 80 logical units
  // We use a safe estimate based on the styling.
  const headerBottomY = headerLayout ? (headerLayout.y + headerLayout.height) : (insets.top + dynamicVerticalScale(80));

  // 2. Bottom Nav Top Y
  // BottomNav = padding(15) + button(44) ~ 60-70 logical units
  const bottomNavTopY = height - insets.bottom - dynamicVerticalScale(80);

  // 3. Stage Area
  const stageHeight = bottomNavTopY - headerBottomY;
  
  // 4. Element Centers (Flexbox space-around emulation)
  // We calculate exact Y positions based on element heights and available space
  ...
  // space-around: 3 items => 3 full spaces distributed as 0.5, 1, 1, 0.5
  // space = totalFreeSpace / 3
  ...
  const bottomRowYComputed = headerBottomY + (spacing * 2.5) + topRowHeight + hubHeight + (bottomRowHeight / 2);
  const bottomRowY = bottomRowYComputed;

  // 5. SVG Coordinates
  // The Hub SVG is inside a scale(1200) view, but the internal coordinates are relative to the <G> origin.
  // The <G> origin is at (480, 480) inside the 1200x1200 viewBox.
  // The Hub Center is at (120, 120) relative to <G>.
  // We need to map the screen pixel difference to SVG units.
  const scaleFactor = dynamicScale(1); // 1 SVG unit = scale(1) screen pixels
  // Small epsilon nudges (SVG units and px) to correct tiny rounding gaps when measurements are available
  // Increased epsilon nudges to better close sub-pixel gaps on high-DPI displays
  const EPS_SVG = 1.5; // ~1.5 SVG units
  const EPS_PX = 1.5; // ~1.5 device pixels
  
  const topBundleY = 120 + ((topRowY - hubCenterY) / scaleFactor);
  const bottomBundleY = 120 + ((bottomRowY - hubCenterY) / scaleFactor);

  // If we measured the exact hub origin and button centers, convert those screen Y positions to SVG units
  ...
  const leftBtnBundleYSvg = leftBtnLayout ? (120 + ((leftBtnCenterScreenY - hubOriginScreenY) / scaleFactor)) : bottomBundleY;
  const rightBtnBundleYSvg = rightBtnLayout ? (120 + ((rightBtnCenterScreenY - hubOriginScreenY) / scaleFactor)) : bottomBundleY;
  ...
  useEffect(() => {
    ...
    const leftId = leftBundleYAnim.addListener(({ value }) => setLeftBundleYDisplay(value));
    const rightId = rightBundleYAnim.addListener(({ value }) => setRightBundleYDisplay(value));
    return () => {
      try { leftBundleYAnim.removeListener(leftId); } catch {}
      try { rightBundleYAnim.removeListener(rightId); } catch {}
    };
  ...
  useEffect(() => {
    ...
    Animated.timing(leftBundleYAnim, {
      toValue: targetLeft,
      duration: DUR,
      easing: Easing.out(Easing.cubic),
    ...
    Animated.timing(rightBundleYAnim, {
      toValue: targetRight,
      duration: DUR,
      easing: Easing.out(Easing.cubic),
      useNativeDriver: false,
  ...
  // X-Axis Dynamic Calculation for Bundles
  // Screen Center X = width / 2
  // Left Button Center X = paddingHorizontal + (buttonWidth / 2)
  // Right Button Center X = width - paddingHorizontal - (buttonWidth / 2)
  
  // Total Padding = Container Padding (12) + ActionRow Padding (12) = 24
  const paddingX = dynamicScale(24);
  const buttonWidth = dynamicScale(100);
  // Prefer measured centers (via refs) when available, fallback to estimated centers
  const leftBtnCenterScreenX = leftBtnLayout ? (leftBtnLayout.x + leftBtnLayout.width / 2) : (paddingX + (buttonWidth / 2));
  const rightBtnCenterScreenX = rightBtnLayout ? (rightBtnLayout.x + rightBtnLayout.width / 2) : (width - paddingX - (buttonWidth / 2));
  
  // Calculate offset from screen center (width/2)
  // SVG Center X (relative to G) is 120.
  // If button is to the left of screen center, offset is negative.
  ...
  // Nudge endpoints toward the actual measured button centers to close tiny gaps
  const leftBundleXNudged = leftBtnLayout ? (leftBundleX + (EPS_SVG * Math.sign(leftBtnOffsetX || -1))) : leftBundleX;
  const rightBundleXNudged = rightBtnLayout ? (rightBundleX + (EPS_SVG * Math.sign(rightBtnOffsetX || 1))) : rightBundleX;

  // Update primary variables so existing JSX usage stays the same but uses nudged values
  leftBundleX = leftBundleXNudged;
  rightBundleX = rightBundleXNudged;

  // Screen Edges in SVG Coordinates for Mid Traces
  const screenLeftEdgeSVG = 120 + ((0 - (width / 2)) / scaleFactor);
  const screenRightEdgeSVG = 120 + ((width - (width / 2)) / scaleFactor);

  // 6. Spine Limits
  // Top: Start from the vertical center of the header section
  const headerHeight = headerLayout ? headerLayout.height : dynamicVerticalScale(80); // matches headerBottomY logic
  const spineTop = headerLayout ? (headerLayout.y + (headerHeight / 2)) : (insets.top + (headerHeight / 2));
  // Bottom: End at the vertical center of the power button (use measured power center when available)
  ...
  const measureButtons = () => {
    try {
      if (leftBtnRef.current && leftBtnRef.current.measure) {
        leftBtnRef.current.measure((fx: number, fy: number, w: number, h: number, px: number, py: number) => {
          setLeftBtnLayout({ x: px, y: py, width: w, height: h });
        });
      }
      if (rightBtnRef.current && rightBtnRef.current.measure) {
        rightBtnRef.current.measure((fx: number, fy: number, w: number, h: number, px: number, py: number) => {
          setRightBtnLayout({ x: px, y: py, width: w, height: h });
        });
      }
      // measure header, hub and power button if possible
      if (headerRef.current && headerRef.current.measure) {
        headerRef.current.measure((fx: number, fy: number, w: number, h: number, px: number, py: number) => {
          setHeaderLayout({ x: px, y: py, width: w, height: h });
        });
      }
      if (hubRef.current && hubRef.current.measure) {
        hubRef.current.measure((fx: number, fy: number, w: number, h: number, px: number, py: number) => {
          setHubLayout({ x: px, y: py, width: w, height: h });
        });
      }
      if (hubOriginRef.current && hubOriginRef.current.measure) {
        hubOriginRef.current.measure((fx: number, fy: number, w: number, h: number, px: number, py: number) => {
          setHubOriginLayout({ x: px, y: py, width: w, height: h });
        });
      }
      if (headerInnerRef.current && headerInnerRef.current.measure) {
        headerInnerRef.current.measure((fx: number, fy: number, w: number, h: number, px: number, py: number) => {
          setHeaderInnerLayout({ x: px, y: py, width: w, height: h });
        });
      }
      if (powerBtnRef.current && powerBtnRef.current.measure) {
        powerBtnRef.current.measure((fx: number, fy: number, w: number, h: number, px: number, py: number) => {
          setPowerLayout({ x: px, y: py, width: w, height: h });
        });
      }
    } catch {
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(measureButtons, 80);
    const t2 = setTimeout(measureButtons, 220);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
    };
  ...
  useEffect(() => {
    const runMeasure = () => {
      ...
      if (typeof window !== 'undefined' && (window as any).requestAnimationFrame) {
        (window as any).requestAnimationFrame(() => measureButtons());
      } else {
